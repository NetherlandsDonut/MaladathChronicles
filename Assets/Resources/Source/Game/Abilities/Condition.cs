...
public class Condition
{
    ...
    public bool IsMet(SaveGame save, Board board)
    {
        if (content == null) return true;
        var c = content.ToLower();
        if (c == "") return true;
        else if (c == "all") return subs == null || subs.Count == 0 || subs.All(x => x.IsMet(save, board));
        else if (c == "any") return subs == null || subs.Count == 0 || subs.Any(x => x.IsMet(save, board));
        else
        {
